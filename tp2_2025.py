...
def agregar_variables(prob, instancia, version_modelo, deseable):
    ...
    nombres_Xij = [f"X_{i+1}_{j+1}" for i in range(n) for j in range(n) if i != j]
...
def agregar_restricciones(prob, instancia, version_modelo, deseables):
    ...
    if (version_modelo):
        ...
        for i in range(n):
            idx = [f"X_{i+1}_{j+1}" for j in range(n) if i!=j]
            idx_inv = [f"X_{j+1}_{i+1}" for j in range(n) if i!=j]
        ...
        for i in range(1, n):
            for j in range(1,n):
                if i!=j:
                    prob.linear_constraints.add(lin_expr=[ [[f"U_{i+1}", f"U_{j+1}", f"X_{i+1}_{j+1}"], [1, -1, n-1]]],
                                                senses=['L'], 
                                                rhs=[n-2],
    ...
    # Restricciones Modelo Completo
        ...
        idx_out = [f"X_1_{j+1}" for j in range(1, n)]
        ...
        idx_in = [f"X_{i+1}_1" for i in range(1, n)]
        ...
        for i in range(1, n):
            idx_out = [f"X_{i+1}_{j+1}" for j in range(n) if j != i]
            idx_in = [f"X_{j+1}_{i+1}" for j in range(n) if j != i]
            expr = idx_out + idx_in
        ...
        for j in range(1, n):
            idx_X = [f"X_{i+1}_{j+1}" for i in range(n) if i != j]
            idx_Y = [f"Y_{i+1}_{j+1}" for i in range(n) if i != j and j+1 in instancia.pares_Y[i+1]]
            expr = idx_X + idx_Y
        ...
        for i in range(n):
            for j in instancia.pares_Y[i+1]:
                idx_stop = [f"X_{k+1}_{i+1}" for k in range(n) if k != i]
                expr = [f"Y_{i+1}_{j}"] + idx_stop
                coefs = [1.0] + [-1.0]*len(idx_stop)
                prob.linear_constraints.add(lin_expr=[[expr, coefs]], 
                                            senses=['L'], 
                                            rhs=[0.0], 
        ...
        for i in range(n):
            idxs = [f"Y_{i+1}_{j}" for j in instancia.pares_Y[i+1] if (j in instancia.refrigerados)]
            if idxs:
    ...
    if (deseables):
        ...
        for i in range(n):
            idxs = [f"Y_{i+1}_{j}" for j in instancia.pares_Y[i+1]]
            if idxs:
...
def mostrar_solucion(prob, instancia):
    ...
    valores = []
    for i in range(n):
        valor = (prob.solution.get_values(f"U_{i+1}"))
        valores.append( (i+1, valor))

    valores_ordenado = sorted(valores, key = lambda x: x[1])
    for cliente, _ in valores_ordenado:
